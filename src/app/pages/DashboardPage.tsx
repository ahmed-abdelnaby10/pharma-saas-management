...
import { TrendingUp, TrendingDown, Package, AlertCircle, Calendar, ShoppingCart, DollarSign, FileText, Plus, Pill, Sparkles, ArrowUpRight, XCircle } from 'lucide-react';
...
const topProducts = [
  { id: 1, name: 'Paracetamol 500mg', category: 'Medicine', sales: 245, revenue: 2450, trend: 12 },
  { id: 2, name: 'Vitamin D3', category: 'Medicine', sales: 198, revenue: 3960, trend: 8 },
  { id: 3, name: 'Moisturizer Cream', category: 'Cosmetic', sales: 167, revenue: 3340, trend: -3 },
  { id: 4, name: 'Antibiotic Syrup', category: 'Medicine', sales: 134, revenue: 4020, trend: 15 },
  { id: 5, name: 'Sunscreen SPF 50', category: 'Cosmetic', sales: 112, revenue: 2800, trend: 5 },
];

const recentActivity = [
  { id: 1, type: 'sale', description: 'Sale completed - Invoice #1247', time: '5 min ago', amount: 125 },
  { id: 2, type: 'stock', description: 'Low stock alert - Paracetamol', time: '12 min ago' },
  { id: 3, type: 'order', description: 'Purchase order received', time: '1 hour ago', amount: 5400 },
  { id: 4, type: 'expiry', description: 'Near expiry alert - 8 items', time: '2 hours ago' },
  { id: 5, type: 'sale', description: 'Sale completed - Invoice #1246', time: '3 hours ago', amount: 87 },
];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">{currentBranch?.name} - March 9, 2026</p>
        </div>
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700 group-hover:text-[#0F5C47]">{t('startPOS')}</span>
          </button>
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700 group-hover:text-[#0F5C47]">{t('startShift')}</span>
          </button>
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700 group-hover:text-[#0F5C47]">{t('addProduct')}</span>
          </button>
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700 group-hover:text-[#0F5C47]">{t('createPO')}</span>
          </button>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={salesData}>
              ...
              <YAxis stroke="#999" />
              <Tooltip />
              <Area type="monotone" dataKey="sales" stroke="#0F5C47" strokeWidth={2} fillOpacity={1} fill="url(#colorSales)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
            ...
            {recentActivity.map((activity) => (
              <div key={activity.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                  ...
                  {activity.type === 'order' && <FileText className="w-4 h-4 text-blue-600" />}
                  {activity.type === 'expiry' && <AlertCircle className="w-4 h-4 text-red-600" />}
                </div>
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-right py-3 px-4 text-sm font-medium text-gray-600 whitespace-nowrap">Trend</th>
              </tr>
            </thead>
            <tbody>
              {topProducts.map((product) => (
                <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-900 text-right">${product.revenue}</td>
