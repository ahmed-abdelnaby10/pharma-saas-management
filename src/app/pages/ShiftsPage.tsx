...
import { CashTransactionModal } from '../components/modals';

const mockShifts = [
  { id: '1', user: 'Ahmed Al-Rashid', branch: 'Main Branch', start: '2026-03-09 09:00', end: '2026-03-09 17:00', sales: 6842, status: 'Closed' },
  { id: '2', user: 'Sarah Johnson', branch: 'North Branch', start: '2026-03-08 09:00', end: '2026-03-08 17:00', sales: 5234, status: 'Closed' },
];
...
export function ShiftsPage() {
  ...
  const handleCashTransaction = (transaction: { amount: number; reason: string; type: 'in' | 'out' }) => {
    ...
    alert(`Cash ${transaction.type === 'in' ? 'In' : 'Out'}: $${transaction.amount}\nReason: ${transaction.reason}`);
        ...
        <div className="bg-gradient-to-r from-[#0F5C47] to-[#1a8c6b] rounded-xl p-6 text-white">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-white/80">Started at {currentShift.startTime.toLocaleTimeString()}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-white/10 rounded-lg p-4">
              ...
              <p className="text-2xl font-semibold">${currentShift.startingCash}</p>
            </div>
            <div className="bg-white/10 rounded-lg p-4">
            ...
            <tbody>
              {mockShifts.map((shift) => (
                <tr key={shift.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4 text-sm font-medium text-gray-900 whitespace-nowrap">{shift.user}</td>
                  <td className="py-3 px-4 text-sm text-gray-600 whitespace-nowrap">{shift.branch}</td>
                  <td className="py-3 px-4 text-sm text-gray-600 whitespace-nowrap">{shift.start}</td>
                  <td className="py-3 px-4 text-sm text-gray-600 whitespace-nowrap">{shift.end}</td>
                  <td className="py-3 px-4 text-sm text-gray-900 text-right whitespace-nowrap">${shift.sales}</td>
                  ...
                  <td className="py-3 px-4">
                    <button className="text-sm text-[#0F5C47] hover:text-[#0d4a39] font-medium">{t('view')}</button>
                  </td>
                </tr>
              ))}
            </tbody>
